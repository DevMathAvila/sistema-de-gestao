  ...
  return (
    <div className="relative">
      <div className={`grid ${compact ? 'grid-cols-2' : 'grid-cols-1 sm:grid-cols-2'} gap-2`}>
        ...
        <button type="button" className={tileClass} onClick={() => { setOpen(true); }}>
          <p className="text-[9px] font-black uppercase tracking-widest opacity-60">Até</p>
          <p className="text-xs font-black mt-1">{labels.fim}</p>
        </button>
      </div>

      {!isMobile && open && (
        <div className={`absolute right-0 mt-2 w-[320px] rounded-3xl p-4 z-30 ${popClass}`}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2 text-red-600">
              <CalendarDays size={14} />
              <span className="text-[10px] font-black uppercase tracking-widest">Filtro de Data</span>
            </div>
            <button type="button" onClick={() => setOpen(false)} className="opacity-60 hover:opacity-100"><X size={14} /></button>
          </div>
          <div className="space-y-3">
            <div>
              <p className="text-[9px] font-black uppercase opacity-60 mb-1">De</p>
              <input type="date" value={dataInicio} onChange={(e) => onChangeField('inicio', e.target.value)} className={tileClass} />
            </div>
            <div>
              <p className="text-[9px] font-black uppercase opacity-60 mb-1">Até</p>
              <input type="date" value={dataFim} onChange={(e) => onChangeField('fim', e.target.value)} className={tileClass} />
            </div>
            <button type="button" onClick={clearAll} className="w-full h-11 rounded-2xl border border-red-600/40 text-red-600 font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2">
          ...
          <div className={`w-full rounded-[2rem] p-5 ${popClass}`}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2 text-red-600">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">Selecionar Período</span>
              </div>
            ...
            <div className="space-y-3">
              <button type="button" className={tileClass}>
                ...
                <input type="date" value={dataInicio} onChange={(e) => onChangeField('inicio', e.target.value)} className="mt-2 w-full bg-transparent text-sm font-black outline-none" />
              </button>
              <button type="button" className={tileClass}>
                <p className="text-[9px] font-black uppercase opacity-60">Até</p>
                <input type="date" value={dataFim} onChange={(e) => onChangeField('fim', e.target.value)} className="mt-2 w-full bg-transparent text-sm font-black outline-none" />
              </button>
              <div className="grid grid-cols-2 gap-2">
                ...
                <button type="button" onClick={() => setOpen(false)} className="h-12 rounded-2xl bg-red-600 text-white font-black text-[10px] uppercase tracking-widest">
                  Aplicar
                </button>
              </div>
            </div>
