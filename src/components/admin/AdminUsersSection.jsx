  ...
  return (
    <section className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl">
      ...
      <div className={`${s.card} p-8 rounded-[2.5rem] mb-10`}>
        <form onSubmit={onCriarUsuario} className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <tr key={u.id} className="hover:bg-red-600/[0.02] transition-colors">
                ...
                {isMaster && (
                  <td className="p-6 text-right">
                    <button onClick={() => onRemoverUsuario(u.id)} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all"><Trash2 size={18} /></button>
                  </td>
                )}
              </tr>
            ))}
          ...
          {usuarios.map((u) => (
            <div key={u.id} className={`${theme === 'dark' ? 'bg-white/[0.03] border-white/10' : 'bg-white border-slate-200'} border rounded-2xl p-4 shadow-sm`}>
              <div className="flex items-center justify-between mb-3">
                ...
                {isMaster && (
                  <button onClick={() => onRemoverUsuario(u.id)} className="h-11 w-11 rounded-xl bg-red-600/10 text-red-600 flex items-center justify-center active:scale-95 transition-all">
                    <Trash2 size={16} />
                  </button>
                )}
              </div>
