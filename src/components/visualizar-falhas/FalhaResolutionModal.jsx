...
import { ArrowRight, Clock3, Monitor, ShieldAlert } from 'lucide-react';

export default function FalhaResolutionModal({
  theme,
  colors,
  modalData,
  etapaFechamento,
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className={`${theme === 'dark' ? 'bg-[#0A0A0A]' : 'bg-white'} border border-white/10 w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl animate-in zoom-in-95`}>
        <div className={`p-6 flex justify-between items-center ${etapaFechamento ? 'bg-green-500/10' : 'bg-red-500/10'}`}>
          ...
          <button onClick={fecharModal} className="text-gray-500 hover:text-red-500">x</button>
        </div>
          ...
          {!etapaFechamento ? (
                    ...
                    return (
                      <button
                        ...
                        className={`w-full p-3 rounded-xl border flex items-center justify-between text-left transition-all ${
                          checked ? 'border-red-600 bg-red-600/10' : (theme === 'dark' ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-slate-50')
                        }`}
                      >
            ...
            <div className="space-y-4">
              ...
              <textarea
                ...
                className={`w-full ${colors.input} p-4 rounded-xl outline-none min-h-[100px] text-[11px] resize-none transition-all`}
                value={solucaoTexto}
