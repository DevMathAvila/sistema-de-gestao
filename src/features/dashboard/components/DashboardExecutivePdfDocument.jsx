...
export default function DashboardExecutivePdfDocument({ metrics, periodoLabel }) {
  ...
  const statusSiga = `${metrics?.sigaResumo?.chamadosPendentes || 0} pendentes / ${metrics?.sigaResumo?.chamadosFechados || 0} concluídos`;
      ...
      <PdfPage>
        <header className="mb-8 flex items-start justify-between gap-6 border-b border-slate-200 pb-6">
          <div className="max-w-[70%]">
            <p className="mb-3 text-[10px] font-black uppercase tracking-[0.28em] text-red-600">Relatório Executivo</p>
            ...
            <p className="mt-4 text-sm font-medium text-slate-500">Período analisado: {periodoLabel}</p>
          </div>
          <LenovoMark />
        </header>
        ...
        <section className="mb-8 grid grid-cols-2 gap-4">
          <KpiCard
            ...
            accent="red"
            helper={`Período consolidado com ${metrics?.chamadosInseridosNoSistema || 0} chamados registrados`}
          />
          <KpiCard
          ...
          <KpiCard
            ...
            accent="gray"
            helper={`${metrics?.topCategory?.value || 0} ocorrências no período`}
          />
          <KpiCard
            label="Status SIGA"
            value={statusSiga}
        ...
        <section className="grid grid-cols-[1.05fr_0.95fr] gap-5">
          <div className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-4">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Distribuição por Tipo de Operação</p>
              <p className="text-sm font-semibold text-slate-700">Visão consolidada por categoria operacional</p>
            </div>
          ...
          <div className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-4">
              ...
              <p className="text-sm font-semibold text-slate-700">Torres comparativas das maiores concentrações</p>
            </div>
      ...
      <PdfPage>
        ...
        <section className="mt-6 grid grid-cols-[1.1fr_0.9fr] gap-5">
          <div className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm font-semibold text-slate-700">Chamados priorizados para acompanhamento executivo</p>
              </div>
            </div>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-slate-50 text-left">
                    ...
                    <th className="px-4 py-3 text-[9px] font-black uppercase tracking-[0.18em] text-slate-500">Descrição Curta</th>
                ...
                <tbody>
                  {(metrics?.sigaTrackingRows || []).slice(0, 8).map((row) => (
          ...
          <div className="space-y-5">
            <div className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Conversão Operacional</p>
              <p className="mt-2 text-5xl font-black italic text-red-600">{conversionRate.toFixed(1)}%</p>
              <p className="mt-3 text-sm font-medium text-slate-600">
                Resolução consolidada frente ao volume total de atividades do período.
              </p>
            </div>

            <div className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Top Setores com Pendências</p>
              <div className="mt-4 space-y-3">
                {(metrics?.setorStatusResumo || []).slice(0, 4).map((item) => (
                  <div key={item.setor} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                    ...
                    <p className="mt-2 text-[10px] font-medium text-slate-500">{item.concluidas} concluídas no período</p>
                  </div>
                ))}
              </div>
            </div>
