...
import { useMonitorTvPage } from '../hooks/useMonitorTvPage';

export default function MonitorTvPage() {
  const { stats, painel } = useMonitorTvPage();
  ...
  return (
    <div className="h-screen w-screen bg-black text-white overflow-hidden p-2 flex flex-col font-sans">
      ...
      <div className="flex-1 overflow-hidden">
        {painel.length === 0 ? (
          <div className="h-full flex items-center justify-center text-zinc-800 font-black text-4xl uppercase italic">Linha Nominal</div>
        ) : (
          <div className={`h-full grid gap-2 ${painel.length <= 4 ? 'grid-cols-2' : 'grid-cols-3'} auto-rows-fr`}>
            {painel.map((s) => (
              <div key={s.nome} className={`flex flex-col rounded-xl border-2 overflow-hidden ${s.critico ? 'border-red-600 bg-red-950/20 animate-pulse' : 'border-zinc-800 bg-zinc-900/50'}`}>
