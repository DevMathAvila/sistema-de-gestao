...
export default function AdminStatsTab({ s, theme, setorFiltro, setSetorFiltro, falhasStats }) {
  return (
    <section className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
        <div>
          <h2 className="text-4xl font-black uppercase italic tracking-tighter">
            Grafico <span className="text-red-600">Pareto</span>
          </h2>
          ...
          {falhasStats.map((item) => (
            <div key={item.nome} className="group">
              ...
              <div className={`w-full h-4 ${theme === 'dark' ? 'bg-white/5' : 'bg-slate-100'} rounded-full overflow-hidden p-1`}>
                <div
                  className="h-full bg-gradient-to-r from-red-600 to-rose-400 rounded-full transition-all duration-1000 group-hover:brightness-110 shadow-lg shadow-red-600/20"
